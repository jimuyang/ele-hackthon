...
public class AStar {
    ...
    public List<Node> search(Coordinate src, Coordinate dest) {
        return search(src.getX(), src.getY(), dest.getX(), dest.getY(), -1);
    }


    //查找坐标（-1：错误，0：没找到，1：找到了）
    public List<Node> search(int x1, int y1, int x2, int y2, int flag) {
